...
export class ProfitGraphComponent implements OnInit {
  ...
  constructor(private _boatOwnerService: BoatOwnerService, private _cottageOwnerService: CottageOwnerService, private _route: ActivatedRoute, private cottageService: CottageService, private router: Router) { this.role = localStorage.getItem('role'); }
  ...
  ngOnInit(): void {
    ...
    if(this.role === "ROLE_BOAT_OWNER"){
      this._boatOwnerService.getProfit().subscribe({
        next: data => {
          this.profit = data
          var temp = new Date(this.profit.first.start)
          this.profit.first.start = temp.getFullYear() + "-" + (temp.getMonth() + 1)
